...
// 5.0.0 Max interface added
// 6.0.0 combined with relay output
#define THIS_VERSION "6.0.0"
...
// #define TEENSY_TOUCH_PIN 14

// digital input pins for Azoteq sensors (note that analog input A0 is pin 14)
#define AZOTEQ_PROX_PIN A1   
#define AZOTEQ_TOUCH_PIN A0  
...
uint8_t ext_led_brightness = 255;
uint8_t ext_standby_palette = 34;
uint8_t ext_active_palette = 35;
CRGBPalette16 standby_palette( gGradientPalettes[ext_standby_palette] ); //34
CRGBPalette16 active_palette( gGradientPalettes[ext_active_palette] ); //35

uint8_t MAX_PALETTE_CHANGES_PER_BLEND = 36;//36



// time of increased brightness after signal, in milliseconds. 
unsigned long TIMEOUT_MILLIS = 500; //500

// twinkle boost
uint8_t twinkle_boost;
uint8_t ext_twinkle_boost_standby = 90; //90
uint8_t twinkle_boost_active = 200; //200
...
uint8_t min_brightness_active = 5; //5
uint8_t max_brightness_active = 30; //30

// brightness range inactive (if want dark after installation, set both to 0)
uint8_t ext_min_brightness_standby = 1;
uint8_t ext_max_brightness_standby = 2;

unsigned int LOGGING_PERIOD_IN_MILLIS = 50;  // not too small, around 500 is good
...
unsigned int num_blending_calls_to_active = 8; //8
unsigned int num_blending_calls_to_standby = 6; //6

unsigned int blending_period_to_active = 2; //2
unsigned int blending_period_to_standby = 2; //2


// --- END ADJUSTABLE PARAMETERS

unsigned int NUM_BLENDING_CALLS_PER_LOOP = 1; //1
unsigned int BLENDING_PERIOD_IN_MILLIS = 40; //40

// ramp for LED speed
int ramp_increment = 2;
int ramp = 0;
...
unsigned int min_brightness = 80; //80
unsigned int max_brightness = 120; //120
...
int analog_constrain = 0;            // the average
int analog_smooth = 0;
int analog_invert = 0;

// Edge detection
int proxCounter = 0;  // counter for the number of button presses
int proxState = 0;        // current state of the button
int lastProxState = 0;    // previous state of the button
int randomDirection = 1;

unsigned int twinkle_speed_min = 10; //minimum speed of LED's (10 - 1000)
unsigned int twinkle_speed_max = 1000; //maximum speed of LED's (10 - 1000)
...
uint8_t ext_tester = 0;

unsigned int ext_analog_min = 15;
unsigned int ext_analog_max = 790;
unsigned int ext_analog_thresh = 200;


// Relay variables
// const int TOUCH_PIN = 8; //azotek IQS231A
const int RELAY_ON_ANALOG = 0;
const int RELAY_OFF_ANALOG = 255;
const int RELAY_OUTPUT_PIN = 9;    // connected to the relay

long lastMist = 0;
long mistDuration = 500;
long mistSpacing = -1;
long mistStarted = 0;
...
void loop()
{
  ...
  bool touch_sensed = !digitalRead(AZOTEQ_TOUCH_PIN);
  ...
  if (currently_misting){
    Serial.println("mist");
      ...
      if (timeSinceMistStarted > mistDuration){
        analogWrite(RELAY_OUTPUT_PIN, RELAY_OFF_ANALOG);
        lastMist = millis();
        currently_misting = 0;
      ...
      if (( touch_sensed ) && timeSinceMist > mistSpacing){
          currently_misting = 1;  
          analogWrite(RELAY_OUTPUT_PIN, RELAY_ON_ANALOG);
          mistStarted = millis();
      }
  }
  ...
  // read & parse from Serial input
  if (Serial.available() > 0) {
    if (Serial.read() == 'C') {    // string should start with C
      ext_led_brightness = Serial.parseInt();   // 
      ext_standby_palette = Serial.parseInt();
      ext_active_palette = Serial.parseInt();
      twinkle_speed_min = Serial.parseInt();
      twinkle_speed_max = Serial.parseInt();
      ext_min_brightness_standby = Serial.parseInt();
      ext_max_brightness_standby = Serial.parseInt();
      ext_twinkle_boost_standby = Serial.parseInt();
      ext_tester = Serial.parseInt();
      ext_analog_min = Serial.parseInt();
      ext_analog_max = Serial.parseInt();
      ext_analog_thresh = Serial.parseInt();
    }
  }
  ...
  EVERY_N_MILLISECONDS(LOGGING_PERIOD_IN_MILLIS) {
    ...
    //Serial.print("Analog: ");
    padding(analog_invert, 4);
    Serial.print(":");
    padding(proxCounter, 6);
    Serial.println();
    //Serial.print("ramp: ");
    //Serial.println(ramp);
...
//  }
  ...
  FastLED.setBrightness(ext_led_brightness);
  FastLED.show();
...
void get_brightness(){
    ...
    // edgedetection
    if (prox != lastProxState) {
      if (prox == 1) {
        proxCounter = (proxCounter + 1) % 99999;
        randomDirection = (random(0, 2) * 2)-1;
      } 
    ...
    // smooth the analog input using array avaraging
    ...
    // calculate the smoothing average:
    analog_smooth = total / numReadings;    
    analog_constrain = constrain(analog_smooth,ext_analog_min,ext_analog_max);
    analog_invert = map(analog_constrain,ext_analog_max,ext_analog_min,ext_analog_min,ext_analog_max);
    //analog_invert = analog_smooth;
    
    // calculate the ramp for LED movement
    ramp_increment = map(analog_invert, ext_analog_min, ext_analog_max, twinkle_speed_min , twinkle_speed_max);
    ramp = constrain(ramp + ((ramp_increment * 0.1)*randomDirection) + random(-1, 2), 0, 1024);
      if (ramp > 1023 ){
        ramp = 0;
      } else if (ramp < 1) {
        ramp = 1024;
    ...
    //if (prox || touch ){
    if (analog_invert >= ext_analog_thresh+1 ){
    ...
    if (millis_since_last_event < TIMEOUT_MILLIS){
      ...
      min_brightness = ext_min_brightness_standby;
      max_brightness = ext_max_brightness_standby;
      twinkle_boost = ext_twinkle_boost_standby;
...
//        led_boost[active_pixel] = ext_twinkle_boost_standby;
